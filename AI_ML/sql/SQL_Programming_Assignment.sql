...
SELECT C.catname, COUNT(E.eventid) AS total_num_events
FROM category C
...
SELECT C.catname, AVG(S.pricepaid / S.qtysold) AS avg_ticket_price
FROM category C
...
(
	SELECT E.catid, COUNT(S.salesid) as total_sales, AVG(S.commission) as avg_comission
	FROM sales S
